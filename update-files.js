...
const VERSION_NAMES = JSON.parse(
  await readFile('src/rest/data/meta.json', 'utf8')
).versionMapping

program
  .description(
    'Update rest, webhooks, and github-apps automated pipeline data files.'
  )
  .addOption(
    new Option(
  ...
  .option(
    ...
    'Includes schemas that are marked as `deprecated: true`'
  )
  .option(
    '-u --include-unpublished',
    'Includes schemas that are marked as `published: false`'
  )
...
async function main() {
  ...
  const sourceDirectory =
    sourceRepo === 'github' ? TEMP_BUNDLED_OPENAPI_DIR : REST_DESCRIPTION_DIR
  ...
  if (sourceRepo === 'rest-api-description') {
    ...
    const currentOpenApiVersions = Object.values(allVersions).map(
      (elem) => elem.openApiVersionName
    )
    derefDir.forEach((schema) => {
      // if the schema does not start with a current version name, delete it
      if (
        !currentOpenApiVersions.find((version) => schema.startsWith(version))
      ) {
  ...
  const { restSchemas, webhookSchemas } = await getOpenApiSchemaFiles(
    derefFiles
  )
...
async function getBundledFiles() {
  ...
  try {
    ...
    execSync(`${path.join(GITHUB_REP_DIR, 'bin/openapi')} ${bundleCommand}`, {
      stdio: 'inherit',
    })
  } catch (error) {
    console.error(error)
...
async function validateInputParameters() {
  ...
  if (
    (includeDeprecated || includeUnpublished) &&
    (sourceRepo !== 'github' || versions)
  ) {
...
export async function normalizeDataVersionNames(sourceDirectory) {
  ...
  for (const schema of schemas) {
    ...
    const date = calendar.startsWith('.')
      ? `-${calendar.slice(1)}`
      : `${calendar}`
    const translatedVersion = `${docsCounterpart}${
      date !== '-' ? date : ''
    }.json`
    await rename(
      path.join(sourceDirectory, schema),
      path.join(sourceDirectory, translatedVersion)
    )
  }
}
